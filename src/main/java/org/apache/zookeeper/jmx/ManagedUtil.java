...
public class ManagedUtil {
    ...
    private static final boolean isLog4jJmxEnabled() {
        ...
        try {
            ...
            if (Boolean.getBoolean("zookeeper.jmx.log4j.disable") == true) {
                JulLog.info("Log4j found but jmx support is disabled.");
            } else {
                enabled = true;
                JulLog.info("Log4j found with jmx enabled.");
            }

        } catch (ClassNotFoundException e) {
            JulLog.info("Log4j not found.");
        }
    ...
    //         JulLog.debug("registerLog4jMBeans()");
    ...
    //             }
    //         } catch (Exception e) {
    //             JulLog.error("Problems while registering log4j jmx beans!", e);
