...
public class QuorumHierarchical implements QuorumVerifier {
   ...
   public boolean equals(Object o){
    ...
    public QuorumHierarchical(Properties qp) throws ConfigException {
        parse(qp);
        JulLog.info(serverWeight.size() + ", " + serverGroup.size() + ", " + groupWeight.size());
    }
    ...
    throws ConfigException{
        ...
        JulLog.info("Reading configuration from: " + configFile);
    ...
    private void computeGroupWeight(){
        ...
        for(long weight: groupWeight.values()){
            JulLog.debug("Group weight: " + weight);
            if(weight == ((long) 0)){
                numGroups--;
                JulLog.debug("One zero-weight group: " + 1 + ", " + numGroups);
            }
        }
    }
    ...
    public boolean containsQuorum(Set<Long> set){
        ...
        else JulLog.debug("Set size: " + set.size());
        ...
        for(long gid : expansion.keySet()) {
            JulLog.debug("Group info: " + expansion.get(gid) + ", " + gid + ", " + groupWeight.get(gid));
            if(expansion.get(gid) > (groupWeight.get(gid) / 2) )
                majGroupCounter++;
        }

        JulLog.debug("Majority group counter: " + majGroupCounter + ", " + numGroups);
        if((majGroupCounter > (numGroups / 2))){
            JulLog.debug("Positive set size: " + set.size());
            return true;
        }
        else {
            JulLog.debug("Negative set size: " + set.size());
            return false;
        }
    }
